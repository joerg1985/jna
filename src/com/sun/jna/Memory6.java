 ...
 *
 * <p>
 * In some cases it might be necessary to use memory obtained from <code>malloc</code>. For example, <code>Memory</code>
 * helps accomplish the following idiom:
 * <pre>
...
public class Memory6 extends Pointer {
    ...
    /**
     * Provide a view into the original memory. Keeps an implicit reference to the original to prevent GC.
     */
    private class SharedMemory extends Memory6 {

        public SharedMemory(long offset, long size) {
            this.size = size;
            this.peer = Memory6.this.peer + offset;
        }
        ...
        protected void boundsCheck(long off, long sz) {
            Memory6.this.boundsCheck(this.peer - Memory6.this.peer + off, sz);
        }

        @Override
        public String toString() {
            return super.toString() + " (shared from " + Memory6.this.toString() + ")";
        }
    }
    ...
    /**
     * Provide a view of this memory using the given offset as the base address. The returned {@link Pointer} will have
     * a size equal to that of the original minus the offset.
     *
     ...
     * Maintains a reference to the original {@link Memory} object to avoid GC as long as the shared memory is
     * referenced.
     *
     * @throws IndexOutOfBoundsException if the requested memory is outside the allocated bounds.
     */
     ...
     *
     * @param byteBoundary Align memory to this number of bytes; should be a power of two.
     * @throws IndexOutOfBoundsException if the requested alignment can not be met.
     * @throws IllegalArgumentException if the requested alignment is not a positive power of two.
     */
    public Memory6 align(int byteBoundary) {
        if (byteBoundary <= 0) {
        ...
        for (int i = 0; i < 32; i++) {
            if (byteBoundary == (1 << i)) {
                ...
                if ((peer & mask) != peer) {
                    ...
                    return (Memory6) share(newPeer - peer, newSize);
                }
                return this;
            }
        }
    ...
    protected synchronized void dispose() {
        if (peer == 0) {
            // if someone called dispose the finalizer will run it again and access the synchronized map again
            return;
        }
    ...
    //////////////////////////////////////////////////////////////////////////
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.read</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
    ...
    //////////////////////////////////////////////////////////////////////////
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.write</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    //////////////////////////////////////////////////////////////////////////
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.getByte</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.getShort</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.getInt</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.getLong</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.getFloat</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.getDouble</code>. But this method
     * performs a bounds check to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.getPointer</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Get a ByteBuffer mapped to a portion of this memory. We keep a weak reference to all ByteBuffers provided so that
     * this memory object is not GC'd while there are still implicit outstanding references to it (it'd be nice if we
     * could attach our own reference to the ByteBuffer, but the VM generates the object so we have no control over it).
     *
    ...
    //////////////////////////////////////////////////////////////////////////
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.setByte</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
     * @see Pointer#setByte
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.setChar</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
     * @see Pointer#setChar
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.setShort</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.setInt</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
     * @see Pointer#setInt
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.setLong</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
     * @see Pointer#setLong
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.setFloat</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.setDouble</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la <code>Pointer.setPointer</code>. But this method
     * performs a bounds checks to ensure that the indirection does not cause memory outside the <code>malloc</code>ed
     * space to be accessed.
     *
